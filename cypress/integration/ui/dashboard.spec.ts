describe('Dashboard view tests', () => {
  before(() => {
    cy.login();
    cy.intercept(
      { method: 'GET', url: '**/assets/offered_to/*' },
      { body: {} }
    ).as('offeredTo');
    cy.intercept({ method: 'GET', url: '**/claim/issuer/*' }, { body: {} }).as(
      'issuerClaim'
    );
    cy.intercept(
      {
      ...
      { fixture: 'claims/requester-claims-basic.json' }
    ).as('requesterClaim');
    cy.intercept(
      { method: 'GET', url: '**/DID/*' },
      { fixture: 'did/cached-did.json' }
    ).as('cachedDidDoc');
  });
  ...
  xit('should copy did to clipboard', () => {
    ...
    cy.get('#toast-container .toast-message').contains(
      'User Did successfully copied to clipboard.'
    );
  });
});
