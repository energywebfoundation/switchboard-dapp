...
describe('filterBy organization/application/role', () => {
  ...
  it('should filter at once through organization, application and role', () => {
    ...
    expect(filterBy(data, 'test', '', '', 'iam.ewc')).toEqual([{namespace: 'test.iam.ewc'}]);
    expect(filterBy(data, 'abc', 'test', '', 'iam.ewc')).toEqual([{namespace: `testapp.${NamespaceType.Application}.abc.iam.ewc`}]);
    expect(filterBy(data, 't', '', 'role', 'iam.ewc')).toEqual([{namespace: `testrole.${NamespaceType.Role}.ttt.iam.ewc`}]);
    expect(filterBy(data, 'dsa', 'testapp', 'rola', 'iam.ewc')).toEqual([{namespace: `rolapp.${NamespaceType.Role}.testapp.${NamespaceType.Application}.dsa.iam.ewc`}]);
  });
});
