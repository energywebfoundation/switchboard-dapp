...
describe('Owned Enrolments reducer', () => {
  ...
  describe('updateEnrolmentSuccess action', () => {
    it('should update enrolment', () => {
      ...
      const action = OwnedActions.updateEnrolmentSuccess({enrolment: {isAccepted: true, id: '1'} as EnrolmentClaim});
  ...
  describe('removeEnrolment action', () => {
    it('should remove element from the list', () => {
      const { initialState } = fromReducer;
      const enrolments = [{isAccepted: false, id: '1'}, {isAccepted: true, id: '3'}]  as EnrolmentClaim[];
      const action = OwnedActions.removeEnrolment({enrolment: {isAccepted: true, id: '1'} as EnrolmentClaim});
      const state = fromReducer.reducer({...initialState, enrolments}, action);

      expect(state.enrolments.find(enrolment => enrolment.id === '1')).toBeFalsy();
      expect(state.enrolments.find(enrolment => enrolment.id === '3')).toBeTruthy();
