...
export class RoleCreationService {
  ...
  async areIssuersValid(issuerType: IssuerType, issuerRoleName: string, issuerList: string[]): Promise<boolean> {
    ...
    if (IssuerType.ROLE === issuerType) {
      ...
      const exists = await this.domainsFacade.checkExistenceOfDomain(issuerRoleName);

      if (!exists || !issuerRoleName.includes(`.${NamespaceType.Role}.`)) {
        this.toastrService.error('Issuer Role Namespace does not exist or is invalid.', TOASTR_HEADER);
        return false;
      } else {
        ...
        if (!did || !did.length) {
          this.toastrService.error('Issuer Role has no approved users.', TOASTR_HEADER);
          return false;
        }
      }
    }
