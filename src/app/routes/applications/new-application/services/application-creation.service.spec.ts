...
describe('ApplicationCreationService', () => {
  ...
  let domainsSpy;
  beforeEach(() => {
    toastrSpy = jasmine.createSpyObj('SwitchboardToastrService', ['success', 'error']);
  ...
  it('should return false when domain do not exist', async() => {
    ...
    expect(toastrSpy.error).toHaveBeenCalledWith('Organization namespace does not exist.', undefined);
  });

  it('should return false when domain is undefined', async() => {

    expect(await service.isOrganizationNamespaceAvailable(undefined)).toBeFalse();
    expect(toastrSpy.error).toHaveBeenCalledWith('Organization Namespace is missing.', undefined);
  });

  it('should return false when sub domain is undefined', async() => {
    domainsSpy.checkExistenceOfDomain.and.returnValues(Promise.resolve(true), Promise.resolve(false));

    expect(await service.isOrganizationNamespaceAvailable('test')).toBeFalse();
    expect(toastrSpy.error).toHaveBeenCalledWith('Application subdomain in this organization does not exist.', undefined);
  });

  it('should return false when user is not owner of domain', async() => {
    domainsSpy.checkExistenceOfDomain.and.returnValues(Promise.resolve(true), Promise.resolve(true));
    domainsSpy.isOwner.and.returnValue(Promise.resolve(false));

    expect(await service.isOrganizationNamespaceAvailable('test')).toBeFalse();
    expect(toastrSpy.error).toHaveBeenCalledWith('You are not authorized to create an application in this organization.', undefined);
  });
  ...
  it('should return false when one endpoint will throw an error', async() => {
    domainsSpy.checkExistenceOfDomain.and.returnValues(Promise.reject({message: 'reason'}));
